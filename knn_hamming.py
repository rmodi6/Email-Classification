...
def hammingDistance(instance1, instance2, length):
	...
	for x in range(length):
		#print("Instances: ", instance1[x], instance2[x])
		val=int(int(instance1[x])^int(instance2[x]))
		dist=0
		while(val!=0):
			dist+=1
			val&=val-1
...
def classify(klist, trainingSet, testSet, current_path):
	...
	for x in range(len(testSet)):
		...
		print('Completed ' + str((x+1)*100/len(testSet)) + '%', end = '\r')
...
def main():
	...
	testSet=[]
	split = 0.67
	current_path = "/media/saif/WORK/Engg Stuff/FYP/Our Implementation/Datasets/Set 4/"
